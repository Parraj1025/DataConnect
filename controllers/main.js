        ...
        try {
            inquirer.prompt(questions).then(async (answers) => {
                ...
                values ('${newDept}');`;
                    ...
                    const newDepartment = await addedDept
                    if (newDepartment){
                        dowork()
                    }
                }
            ...
            values ('${newRole}',${newSalary}, ${matchingId});`)
                ...
                if (answers.task == 'add employee') {
                    ...
                    const findManagerId = await client.query(`select id from role where "name" = 'Manager' and department_id = ${deptId};`)
                    const managerId = await findManagerId
                    let assignedManager = ''
                    if (!managerId.rows.length) {
                        assignedManager = null
                    }
                    else{assignedManager = managerId.rows[0].id}
                ...
                if (answers.task == 'view all employees') {
                    ...
                    const empCheck = await allEmps
                    console.table(empCheck.rows)
                    dowork()
                }

                if (answers.task == 'update employee') {
